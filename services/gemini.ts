...
export const enhanceUserPhoto = async (imageUrl?: string, base64Image?: string): Promise<string | null> => {
  try {
    ...
    logger.error("No image returned from Gemini", { response: JSON.stringify(response) });
    return null;
  } catch (error: any) {
    logger.error("Enhancement Error", error);
    if (error?.message?.includes('429') || error?.message?.includes('RESOURCE_EXHAUSTED')) {
      logger.warn('⚠️ Free tier limit reached. Falling back to original image.');
      return null;
    }
    return null;
  }
...
export const searchProducts = async (query: string, gender?: string): Promise<Product[]> => {
  try {
    ...
    const jsonFormat = `[{"index": 0, "name": "Product Name", "brand": "Brand", "price": 100, "category": "top|bottom|shoes|outerwear|one-piece|accessory", "description": "Details", "imageUrl": "url"}]`;
    let prompt = "";
    if (!isFallback && searchResults.length > 0) {
      ...
      prompt = `Extract fashion data from results: ${JSON.stringify(llmInput)}. Return JSON array in this exact format: ${jsonFormat}`;
    } else {
      prompt = `Generate 3 fashion items for query: "${fullQuery}". Return JSON array in this exact format: ${jsonFormat}`;
    }
...
export const generateTryOnImage = async (userPhotoUrl?: string, userPhotoBase64?: string, products?: Product[]): Promise<string | null> => {
  try {
    ...
    logger.error("No image returned from Gemini", { response: JSON.stringify(response) });
    if (products?.[0]?.imageUrl) return products[0].imageUrl;
    return userPhotoUrl || userPhotoBase64 || null;
  } catch (error: any) {
    logger.error("Try-On Error:", error);
    if (error?.message?.includes('429') || error?.message?.includes('RESOURCE_EXHAUSTED')) {
      logger.warn('⚠️ Free tier limit reached. Falling back to product/user image.');
      if (products?.[0]?.imageUrl) return products[0].imageUrl;
      return userPhotoUrl || userPhotoBase64 || null;
    }
    return null;
...
export const generateStealTheLook = async (
  ...
  try {
    ...
    logger.error("No image returned from Gemini for steal the look");
    return inspirationPhotoUrl || inspirationPhotoBase64 || null;
  } catch (error: any) {
    logger.error("Steal the Look Error", error);
    if (error?.message?.includes('429') || error?.message?.includes('RESOURCE_EXHAUSTED')) {
      logger.warn('⚠️ Free tier limit reached. Falling back to inspiration image.');
      return inspirationPhotoUrl || inspirationPhotoBase64 || null;
    }
    return null;
  }
