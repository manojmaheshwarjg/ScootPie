...
export const PhotoUploader: React.FC<PhotoUploaderProps> = ({ onPhotoUploaded, onOpenWaitlist, onCancel }) => {
    ...
    const capturePhoto = () => {
        if (videoRef.current && canvasRef.current) {
            ...
            if (ctx) {
                ...
                console.log(`📷 [Upload] Webcam photo captured (${video.videoWidth}x${video.videoHeight})`);
    ...
    const processFile = (file: File) => {
        ...
        console.log(`📸 [Upload] Processing image file: ${file.name} (${(file.size / 1024).toFixed(1)} KB, ${file.type})`);
        const reader = new FileReader();
        reader.onload = (e) => {
            setTempPhotoData(e.target?.result as string);
            setError(null);
            console.log('✅ [Upload] Image loaded into preview, awaiting gender selection.');
        };
    ...
    const handleGenderSelect = (gender: 'mens' | 'womens' | 'unisex') => {
        if (tempPhotoData) {
            console.log(`🏃 [Upload] Gender selected: "${gender}" — submitting photo for AI enhancement...`);
            onPhotoUploaded({
