...
export const StudioControls: React.FC<StudioControlsProps> = ({
    ...
    const handleGenerate = async () => {
        ...
        try {
            ...
            if (foundProducts.length > 0) {
                ...
                const results = await Promise.all(
                    targetProducts.map(async (newProduct) => {
                        ...
                        console.log(`🎨 [Studio] Generating look for: ${newProduct.name}`);
                        const img = await generateTryOnImage(userPhoto, itemsToWear);
                        if (!img) console.warn(`⚠️ [Studio] No image returned for: ${newProduct.name}`);
                ...
                if (successful.length > 0) {
                    ...
                    console.error('❌ [Studio] All try-ons returned null — check Gemini API key and model availability.');
                    setStatus('ERROR');
            ...
            console.error('❌ [Studio] Unhandled error in handleGenerate:', error);
            setStatus('ERROR');
